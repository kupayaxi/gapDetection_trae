...
def _monitor_detection_process(self):
    ...
    while self._detection_thread_running:
        try:
            # 检查进程是否存活
            if self._detection_process and self._detection_process.poll() is not None:
                # 进程已退出，记录退出码
                exit_code = self._detection_process.returncode
                self._log_manager.log_system_event("检测进程退出", f"进程意外退出，退出码: {exit_code}", level="ERROR")
                ...
                if self._should_restart_process(exit_code):
                    self._log_manager.log_system_event("检测进程重启", "尝试重启检测进程")
                    # 延迟重启，避免立即重启导致资源占用过高
                    time.sleep(1)
            ...
            # 记录监控异常，但不终止监控线程
            self._log_manager.log_system_event("进程监控异常", f"监控线程发生异常: {e!s}", level="ERROR")
    ...
    self._log_manager.log_system_event("进程监控结束", "检测进程监控线程已结束")


def _should_restart_process(self, exit_code):
    """根据退出码判断是否需要重启进程 返回True表示需要重启，False表示不需要重启.
    """
    # 定义可接受自动重启的退出码
...
def start_camera_detection(self, camera_id=0, **kwargs):
    ...
    try:
        ...
        self._detection_thread = threading.Thread(target=self._monitor_detection_process, daemon=True)
        self._detection_thread.start()

        self._log_manager.log_system_event("检测进程启动", f"摄像头检测进程已启动，摄像头ID: {camera_id}")
        return True

    except Exception as e:
        self._log_manager.log_system_event("检测启动异常", f"启动摄像头检测进程时发生异常: {e!s}", level="ERROR")
        # 确保资源清理
        self.stop_camera_detection()
        return False
```
        ...
        for backend in backends:
            try:
                ...
                if ret:
                    ...
                    cap.release()
                    cap = None
                    log_to_csv("摄像头初始化", "读取测试帧失败，尝试下一个后端", level="WARNING")
                ...
                if cap is not None:
                    cap.release()
                    cap = None
                log_to_csv("摄像头初始化", f"使用后端{backend}初始化失败: {e!s}", level="WARNING")
...
def process_video_stream(cap, timeout=30):
    ...
    while True:
        try:
            ...
            # 捕获所有异常，确保程序不会崩溃
            log_to_csv("视频处理错误", f"处理视频流时发生异常: {e!s}", level="ERROR")
...
def check_dependencies():
    """检查必需的依赖是否已安装，如未安装则尝试安装 返回所有成功安装的包列表和失败的包列表.
    """
    # 必需的包列表
    required_packages = [
        "numpy>=1.21.0",
...
def main():
    # 检查依赖
    _installed, missing = check_dependencies()

    if missing:
        print("警告: 以下依赖安装失败，可能会影响程序功能:")
        for package in missing:
            print(f"  - {package}")
    ...
    with self._process_lock:
        ...
        if hasattr(self, "_detection_process") and self._detection_process is not None:
            try:
                ...
                try:
                    self._detection_process.wait(timeout=5)
                    self._log_manager.log_system_event("检测进程停止", "摄像头检测进程已成功终止")
                except subprocess.TimeoutExpired:
                    # 超时后强制终止
                    self._log_manager.log_system_event(
                        "检测进程强制终止", "进程终止超时，正在强制终止", level="WARNING"
                    )
                    self._detection_process.kill()
                ...
                for pipe in [self._detection_process.stdout, self._detection_process.stderr]:
                    if pipe:
                        try:
                            ...
                            # 记录关闭管道时的异常，但不影响后续操作
                            self._log_manager.log_system_event(
                                "资源清理警告", f"关闭进程管道时发生异常: {e!s}", level="WARNING"
                            )

            except Exception as e:
                self._log_manager.log_system_event("进程终止异常", f"停止检测进程时发生异常: {e!s}", level="ERROR")
            finally:
                # 无论如何都将进程对象设置为None
                self._detection_process = None
```
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.cap is not None:
            try:
                ...
                log_to_csv("资源清理错误", f"释放摄像头时发生异常: {e!s}", level="WARNING")
...
def main():
    ...
    try:
        ...
        log_to_csv("程序异常", f"主程序发生异常: {e!s}", level="ERROR")
        print(f"错误: {e!s}")
    finally:
        # 确保关闭所有窗口
...
class MainWindow(QMainWindow):
    ...
    def start_detection(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"启动检测失败: {e!s}")
            self.log_manager.log_system_event("系统错误", f"启动检测失败: {e!s}", level="ERROR")
    ...
    def stop_detection(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"停止检测失败: {e!s}")
            self.log_manager.log_system_event("系统错误", f"停止检测失败: {e!s}", level="ERROR")
...
class DetectionSettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 连接滑动条和数值框
        self.conf_threshold_slider.valueChanged.connect(lambda value: self.conf_threshold_spinbox.setValue(value / 100))
        ...
        # 连接滑动条和数值框
        self.iou_threshold_slider.valueChanged.connect(lambda value: self.iou_threshold_spinbox.setValue(value / 100))
    ...
    def browse_weights_file(self):
        """浏览模型文件."""
        file_path, _ = QFileDialog.getOpenFileName(self, "选择模型文件", ".", "Model Files (*.pt);;All Files (*)")
        if file_path:
    ...
    def get_settings(self):
        ...
        # 获取选中的类别
        selected_classes = [class_id for class_id, checkbox in self.class_checkboxes.items() if checkbox.isChecked()]
