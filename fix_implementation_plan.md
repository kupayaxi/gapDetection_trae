...
def _monitor_detection_process(self):
    ...
    while self._detection_thread_running:
        try:
            ...
            # 记录监控异常，但不终止监控线程
            self._log_manager.log_system_event("进程监控异常", f"监控线程发生异常: {str(e)}", level="ERROR")
...
def start_camera_detection(self, camera_id=0, **kwargs):
    ...
    try:
        ...
        self._log_manager.log_system_event("检测启动异常", f"启动摄像头检测进程时发生异常: {str(e)}", level="ERROR")
        # 确保资源清理
        self.stop_camera_detection()
        return False
```
                ...
                for pipe in [self._detection_process.stdout, self._detection_process.stderr]:
                    if pipe:
                        try:
                            ...
                            # 记录关闭管道时的异常，但不影响后续操作
                            self._log_manager.log_system_event(
                                "资源清理警告", f"关闭进程管道时发生异常: {str(e)}", level="WARNING"
                            )

            except Exception as e:
                self._log_manager.log_system_event("进程终止异常", f"停止检测进程时发生异常: {str(e)}", level="ERROR")
            finally:
                # 无论如何都将进程对象设置为None
                self._detection_process = None
```
