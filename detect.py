...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/detect_debug.log'),
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    try:
                        ...
                        cv2.putText(im0, label, (xyxy[0], xyxy[1] - 10), 
...
def run(weights='yolov5s.pt', source='0', conf_thres=0.4, iou_thres=0.45, classes=None, 
        view_img=True, save_txt=False, no_save=False, max_frames=0, timeout=0):
    """运行目标检测"""
    cap = None
    try:
        logger.info("开始目标检测")
