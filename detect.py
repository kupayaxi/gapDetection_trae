            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    try:
                        ...
                        logger.error(f"绘制边界框失败: {e!s}")
                        logger.error(f"坐标值: {xyxy}")

        return im0
    except Exception as e:
        logger.error(f"标注图像失败: {e!s}")
        logger.error(traceback.format_exc())
        # 如果标注失败，返回原始图像
        return img0
...
def run(
    ...
    try:
        ...
        # 加载模型
        model, device, _stride, names = load_model(weights)
