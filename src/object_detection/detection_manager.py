...
class DetectionManager:
    ...
    def __init__(self, weights_path=None):
        ...
        possible_paths = [
            self.weights_path,
            os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), self.weights_path
            ),
        ...
        self.relevant_classes = config_manager.get(
            "detection.relevant_classes", [0, 24, 25, 26, 28, 39, 41, 67, 27, 77]
        )
        # 默认的COCO数据集类别名称
        self.class_names = {
            0: "person",
        ...
        self.enable_frame_validation = config_manager.get(
            "detection.enable_frame_validation", False
        )  # 默认关闭多帧验证
        ...
        self.small_object_area_threshold = config_manager.get(
            "detection.small_object_area_threshold", 2500
        )  # 50x50像素
    ...
    def load_model(self):
        ...
        try:
            ...
            log_manager.log_error(f"加载模型失败: {e!s}")
    ...
    def start_camera_detection(self):
        ...
        try:
            ...
            detect_script_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "detect.py"
            )
        ...
        while not self.monitor_stop_event.is_set():
            if self.camera_detection_process:
                try:
                    ...
                    if return_code is not None:
                        ...
                        if return_code == 0:
                            ...
                            try:
                                ...
                                log_manager.log_error(f"读取进程输出失败: {read_error!s}")
                    ...
                    log_manager.log_error(f"监控检测进程失败: {e!s}")
                    try:
                        with self.lock:
                            self.camera_detection_running = False
                            self.camera_detection_process = None
    ...
    def stop_camera_detection(self):
        ...
        try:
            ...
            with self.lock:
                ...
                if self.monitor_thread and self.monitor_thread.is_alive():
                    try:
                        ...
                        log_manager.log_error(f"等待监控线程结束失败: {thread_join_error!s}")
                ...
                if self.camera_detection_process:
                    ...
                    try:
                        ...
                        try:
                            ...
                            log_manager.log_error(f"强制终止进程失败: {kill_error!s}")
                    except Exception as terminate_error:
                        log_manager.log_error(f"终止进程失败: {terminate_error!s}")
                    ...
                    try:
                        ...
                        log_manager.log_error(f"关闭进程管道失败: {close_error!s}")
            ...
            log_manager.log_error(f"停止摄像头检测失败: {e!s}")
            log_manager.log_error(traceback.format_exc())  # 添加详细的堆栈跟踪
            try:
                with self.lock:
                    self.camera_detection_running = False
                    ...
                    if current_state in [SystemState.CLOSING, SystemState.CLOSED]:
                        ...
                        if self.roi:
                            ...
                            for obj in detection_result["objects"]:
                                ...
                                if (roi_x1 <= obj_center_x <= roi_x2 and roi_y1 <= obj_center_y <= roi_y2) or not (
                                    obj_x2 < roi_x1 or obj_x1 > roi_x2 or obj_y2 < roi_y1 or obj_y1 > roi_y2
                                ):
                        ...
                        if hasattr(signal_handler, "trigger_alarm"):
                            ...
                            log_manager.log_system_event(
                                "报警触发", f"检测到异物并触发报警: {object_type} 在 {position}"
                            )
            except Exception as alarm_error:
                log_manager.log_error(f"触发报警失败: {alarm_error!s}")
    ...
    def _apply_roi_filtering(self, detection_result, frame):
        ...
        log_manager.log_system_event(
            "ROI筛选", f"原始检测到{len(detection_result['objects'])}个物体，ROI内保留{len(filtered_objects)}个物体"
        )
    ...
    def set_enable_frame_validation(self, enable):
        ...
        if not enable:
            ...
            log_manager.log_system_event(
                "多帧验证设置", f"多帧验证已启用，需要连续{self.frame_validation_count}帧检测到同一物体"
            )
    ...
    def _is_near_roi_edge(self, obj, roi, threshold=20):
        ...
        Returns:
            bool: 是否靠近ROI边缘
        """
        roi_x1, _roi_y1, roi_x2, _roi_y2 = roi
        obj_x1, _obj_y1, obj_x2, _obj_y2 = obj["x1"], obj["y1"], obj["x2"], obj["y2"]
    ...
    def switch_model(self, weights_path):
        ...
        with self.lock:
            try:
                ...
                log_manager.log_error(f"切换模型异常: {e!s}")
                log_manager.log_error(traceback.format_exc())
                return False
