                    ...
                    def get_log_config(self):
                        return {
                            ...
                            "log_file": os.path.join(
                                os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "logs", "system.log"
                            ),
                        }
...
class LogManager:
    ...
    def __init__(self, log_file=None):
        ...
        if log_file is None:
            log_file = getattr(config_mgr, "get", lambda key, default=None: default)(
                "log.file_path", "logs/detection_log.csv"
            )
        self.log_file = Path(log_file)
        self.max_file_size = getattr(config_mgr, "get", lambda key, default=None: default)(
    ...
    def export_logs(self, start_time=None, end_time=None, export_format="csv", export_path=None):
        ...
        try:
            ...
            with open(self.log_file, encoding="utf-8") as source, open(
                export_path, "w", newline="", encoding="utf-8"
            ) as dest:
        ...
        try:
            with open(self.log_file, encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    # 添加到结果列表
                    results.append({"timestamp": row[0], "event_type": row[1], "details": row[2]})
