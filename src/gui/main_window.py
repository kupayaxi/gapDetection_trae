...
class MainWindow(QMainWindow):
    ...
    def create_control_panel(self):
        ...
        # 获取当前使用的模型
        current_model = (
            detection_manager.weights_path.split(os.path.sep)[-1]
            if hasattr(detection_manager, "weights_path")
            else "yolov5s.pt"
        )
    ...
    def setup_connections(self):
        ...
        self.confidence_slider.valueChanged.connect(
            lambda value: config_manager.set("detection.confidence_threshold", value / 100)
        )

        # 多帧验证开关
        self.frame_validation_checkbox.stateChanged.connect(
            lambda state: detection_manager.set_enable_frame_validation(state == Qt.Checked)
        )

        self.frame_count_slider.valueChanged.connect(lambda value: self.frame_count_value.setText(str(value)))
        self.frame_count_slider.valueChanged.connect(
            lambda value: config_manager.set("detection.frame_validation_count", value)
        )
    ...
    def on_model_selection_changed(self, index):
        ...
        if model_name == "自定义...":
            ...
            file_path, _ = QFileDialog.getOpenFileName(self, "选择模型文件", "", "PyTorch 模型 (*.pt);;所有文件 (*)")
    ...
    def switch_detection_model(self, model_path):
        ...
        try:
            ...
            if detection_manager.switch_model(model_path):
                ...
                QMessageBox.critical(self, "错误", "模型切换失败:\n可能是模型文件不存在或格式不正确")
        except Exception as e:
            error_msg = f"模型切换异常: {e!s}"
            self.statusBar().showMessage(error_msg)
            log_manager.log_error(f"模型切换异常: {e!s}")
        ...
        try:
            ...
            if self.roi_enabled and self.roi_coords:
                ...
                if self.roi_editing:
                    ...
                    # 添加提示文本
                    cv2.putText(
                        display_frame, "拖拽调整ROI区域", (x1, y1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2
                    )
            ...
            # 转换为QImage
            height, width, _channel = rgb_image.shape
    ...
    def apply_roi_settings(self):
        ...
        try:
            ...
            print(f"应用ROI设置失败: {e!s}")
            QMessageBox.warning(self, "警告", f"应用ROI设置失败: {e!s}")
